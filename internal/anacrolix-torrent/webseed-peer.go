...
const (
	// V255: Reduced from 5s to 2s for faster recovery during streaming.
	webseedPeerUnhandledErrorSleep   = 2 * time.Second
	...
	for !ws.peer.closed.IsSet() {
		...
		ws.peer.requestState.Requests.Iterate(func(x RequestIndex) bool {
			...
			if errors.Is(err, webseed.ErrTooFast) {
				// V255: Reduced from 10s to 2s for faster recovery during streaming.

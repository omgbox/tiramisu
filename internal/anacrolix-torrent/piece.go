...
type Piece struct {
	...
	// V255: Cached storage.Piece to avoid allocating metainfo.Piece + storage wrapper per call.
	// Initialized in makePieces(), immutable after that.
	cachedStorage storage.Piece

	// This can be locked when the Client lock is taken, but probably not vice versa.
	pendingWritesMutex sync.Mutex
	pendingWrites      int
	noPendingWrites    sync.Cond
	// V255: Atomic mirror of pendingWrites>0 for lock-free fast-path in waitNoPendingWrites.
	atomicPendingWrites atomic.Int32
...
func (p *Piece) Storage() storage.Piece {
	// V255: Return cached storage piece to avoid allocations on every call.
	return p.cachedStorage
}
...
func (p *Piece) incrementPendingWrites() {
	...
	p.pendingWrites++
	p.atomicPendingWrites.Store(int32(p.pendingWrites))
...
func (p *Piece) waitNoPendingWrites() {
	// V255: Fast-path — atomic check avoids mutex lock/unlock (~40ns on ARM)
	// in the common case where no writes are in progress for this piece.
	// During streaming, reads vastly outnumber concurrent writes to the same piece.
	if p.atomicPendingWrites.Load() == 0 {
		return
	}
	p.pendingWritesMutex.Lock()
